...
% This script is a (slightly) modified version of the SPM_CH30 tutorial
% that can be run using the automated testing tools (the version of the
% script used in the tutorial guide requires user customization to run)
...
% and a tasklist file.
%
% The default parameter file will be used
%
% To use a non-default parameter file, set it as the first input to aarecipe:
%
% PARAMETER_FNAME = '/path/to/parameter_xml_file';
% aap = aarecipe(PARAMETER_FNAME, tasklist_fname);

[aahome,~,~] = fileparts(which('aa_ver5'));
tasklist_fname = fullfile(aahome, 'examples', 'tutorials', 'SPM_CH30.xml');
