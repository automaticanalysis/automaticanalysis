...
% Chloe Hutton 
% Tibor Auer (tibor.auer@mr-cbu.cam.ac.uk) lines 140 and 146 - NIFTI4D support
...
if iscell(epi_dir)
   ...
   for sessnum=1:nsessions
      epi_all = strvcat(spm_select('ExtList',epi_dir{sessnum},'^f.*\.nii$',1),spm_select('ExtList',epi_dir{sessnum},'^f.*\.img$',1));
      epi_img{sessnum}=fullfile(epi_dir{sessnum},epi_all(1,:));
   end
elseif ischar(epi_dir)
   nsessions=1;
   sessnum=1;
   epi_all = strvcat(spm_select('ExtList',epi_dir,'^f.*\.nii$',1),spm_select('ExtList',epi_dir,'^f.*\.img$',1));
