...
%=======================================================================
   ...
   case 'write'
      ...
      for v = 1:numel(V)
          img(v) = struct(...
              'fname', spm_file(V(v).fname,'prefix',name),...
              'dim',    V(v).dim(1:3),...
              'dt',     [varargin{5} spm_platform('bigend')],...
              'mat',    V(v).mat,...
              'descrip',varargin{6});
          
          if numel(V) > 1, img(v).fname = spm_file(img(v).fname,'suffix',sprintf('-%06d',v)); end
          
          Vo(v) = spm_write_vol(img(v),vol(:,:,:,v));
      end
      varargout{1} = Vo;
...
%=======================================================================
   ...
   % SPM5 Update
   IP.epiP = spm_vol(spm_select(Inf,'image','Select sample EPI image'));
...
%=======================================================================
   ...
   case 'unwarpepi'
      ...
      msk = reshape(double(xyz2(:,1)>=1 & xyz2(:,1)<=IP.vdmP.dim(1) &...
                   ...
                   xyz2(:,3)>=1 & xyz2(:,3)<=IP.vdmP.dim(3)),IP.epiP(1).dim(1:3));
              
      % Read in voxel displacement map in correct space
      tvdm = reshape(spm_sample_vol(spm_vol(IP.vdmP.fname),xyz2(:,1),...
                      xyz2(:,2),xyz2(:,3),1),IP.epiP(1).dim(1:3));

      % Voxel shift map must be added to the y-coordinates. 
      for v = 1:numel(IP.epiP)
          uepi = reshape(spm_sample_vol(IP.epiP(v),xyz(:,1),...
              xyz(:,2)+tvdm(:),xyz(:,3),1),IP.epiP(v).dim(1:3));% TEMP CHANGE
          ...
          if IP.ajm==1
              if IP.epifm==1 % If EPI, use inverted jacobian
                  ...
                  IP.jim = reshape(spm_sample_vol(IP.vdm.ijac,xyz2(:,1),...
                      xyz2(:,2),xyz2(:,3),1),IP.epiP(v).dim(1:3));
              else
...
%=======================================================================
   ...
   case 'unwarpepixy'
      ...
      for v = 1:numel(IP.epiP)
          uepi = reshape(spm_sample_vol(IP.epiP(v),xyz(:,1)+tvdm(:),...
              xyz(:,2),xyz(:,3),1),IP.epiP(v).dim(1:3));% TEMP CHANGE
