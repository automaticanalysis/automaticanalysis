function img = spm_render(dat,brt,rendfile)
...
try, prevrend.rendfile = rendfile; catch, end;
try, prevrend.brt = brt; catch, end;
...
if ~isfinite(brt),
    ...
    for i=1:length(rend),
        ...
        ax=axes('Parent',Fgraph,'units','normalized',...
            ...
            'Visible','off');
        image(rgb,'Parent',ax);
        img(1:size(rgb,1),1:size(rgb,2),:,i) = img_flipud(rgb);
        set(ax,'DataAspectRatio',[1 1 1], ...
            'PlotBoxAspectRatioMode','auto',...
            'YTick',[],'XTick',[],...
