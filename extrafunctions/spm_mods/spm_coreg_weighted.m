function x = spm_coreg_weighted(varargin)
...
def_flags = struct('sep',[4 2],'params',[0 0 0  0 0 0], 'cost_fun','nmi','fwhm',[7 7],...
	'tol',[0.02 0.02 0.02 0.001 0.001 0.001 0.01 0.01 0.01 0.001 0.001 0.001],'graphics',1,'wgt',0);    %[Peter Koopmans modified, value of wgt does not mean anything here at this moment]
if nargin < 3,
	flags = def_flags;
else
	flags = varargin{3};
...
for k=1:numel(VF),
	...
	xk  = flags.params(:);
	for samp=flags.sep(:)',
		xk     = spm_powell(xk(:), xi,sc,mfilename,VG,VFk,samp, flags.cost_fun, flags.fwhm, flags.wgt);    % [Peter Koopmans modified]
		x(k,:) = xk(:)';
	end
	if flags.graphics,
...
%_______________________________________________________________________
function o = optfun(x,VG,VF,s,cf,fwhm,wgt)
...
H = spm_hist2_weighted(VG.uint8,VF.uint8, VF.mat\spm_matrix(x(:)')*VG.mat, sg, wgt); % [Peter Koopmans modified]
