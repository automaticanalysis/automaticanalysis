...
aap=aarecipe('fmri_advanced.xml');
...
for b = 1:2
    aap.tasksettings.aamod_smooth(b).FWHM = 5; 						% smoothing kernel size, typical value 10
    
    aap.tasksettings.aamod_firstlevel_model(b).xBF.name = 'hrf (with time and dispersion derivatives)';
    aap.tasksettings.aamod_firstlevel_model(b).xBF.UNITS = 'secs';    	% OPTIONS: 'scans'|'secs' for onsets and durations, typical value 'secs'
    aap.tasksettings.aamod_firstlevel_model(b).includemovementpars = 1;% Include/exclude Moco params in/from DM, typical value 1
    aap.tasksettings.aamod_firstlevel_threshold(b).threshold.p = 0.01;
end

aap = aas_renamestream(aap,'aamod_secondlevel_GIFT_00001','epi','aamod_smooth_00002.epi');

%% STUDY
% Directory for analysed data
aap.acq_details.root = fullfile(aap.acq_details.root,'aa_demo');
aap.directory_conventions.analysisid = 'fmri_advanced_test';
...
% The "hard"(-coded) way
aap = aas_addevent(aap,'aamod_firstlevel_model_*','S1','*',...
    'REST',...                                                                                  % name
...
aap = aas_addevent(aap,'aamod_firstlevel_model_*','S2','*',...
    'REST',...                                                                                  % name
...
aap = aas_addcontrast(aap,'aamod_firstlevel_contrasts_*','*','singlesession:Loc',[-1 1],'Loc_RightFinger-Rest','T');
