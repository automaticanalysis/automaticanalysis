...
fprintf('Rendering %s', img);

% set the colormap
if ischar(cmap)
    col = feval(cmap,256);
else    
    col = mycmap(cmap, 256); % make a colormap that SPM is happy with
end
...
fprintf('Range of raw data between %g and %g.', min(min(min(Y))), max(max(max(Y))));
...
fprintf('Range of rendered data between %g and %g.', min(min(min(Y))), max(max(max(Y))));
...
if any(v(:))
  if strcmp(cfg.colorscale, 'symmetrical')
    ...
    fprintf('Color scale from -%g to %g.', maxv, maxv);
  elseif ~isempty(cfg.colorscale)
    ...
    fprintf('Color scale from %g to %g.', cmin, cmax);    
  else
    if size(col,1)>3
