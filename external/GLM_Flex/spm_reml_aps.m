function [V,h,Ph,F,Fa,Fc] = spm_reml_aps(YY,X,Q,N,D,t)
...
%     D
    ...
    %----------------------------------------------------------------------
    if abs(pF) > 1e6
        [V,h,Ph,F,Fa,Fc] = spm_reml_aps(YY,X,Q,N,1,t - 2);
        return
    else
        dF = pF;
    end
...
if ~D
    if min(eig(V)) < 0
        [V,h,Ph,F,Fa,Fc] = spm_reml_aps(YY,X,Q,N,1,2);
        return
    end
end
