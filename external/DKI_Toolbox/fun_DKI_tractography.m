function fun_DKI_tractography(FDir_name,NM_name,Smask_name,sT,...
    Tmask_name,Tm_t,tangle,tmin,tmax,NVDSeeds)
...
[pth,outname, daa]=fileparts(FDir_name);
...
for q = 1:ANSVoxels*NVDSeeds^3;
    ...
    for nfib=1:NM(Int_i,Int_j,Int_k)
        ...
        s=1;
        Temp_Tracts(s,:)= [i,j,k];
        Temp_TractTmask(s,:)= Tmask(Int_i,Int_j,Int_k);
        ...
        if s*step>Tmin_t
            ...
            %TractTmask{t,1} = Temp_TractTmask;
...
save([pth,sc,'Tractography', sc, outname, '_report_tdki'],...
'timerun',...
'NSVoxels',...
'ANSVoxels',...
'Ptracts',...
