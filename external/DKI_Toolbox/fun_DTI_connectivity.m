function fun_DTI_connectivity(V1_name,S_name,sT,T_name,Tm_t,...
    tangle,tmin,tmax,NVDSeeds,ROIname, ROIlabels,WMname,out_name)
%% Constructs brain structural connectivity based on DTI tractography
% Inputs
...
% for binary 
% 4) T_name - directory and folder name (with extention) where FA maps is 
% saved. This map this used for the stopping criteria.
% 5) Tm_t - Value of FA threshold for FA stopping criteria. Tracking will 
...
% 10) ROIname - directory and folder name (with extention) ROIs are saved.
%               different inteer should be a different ROI. 
% 11) ROIlabel - numbers of ROIs of interest 
% 12) WMname - directory of file containing the metric to generate the
% avarege metric through the fibres connecting 2 rois
%
% First version: 16/04/2015 based on DTI tractography (11/04/2015)
...
if nargin<13
    ...
    outputTRK_C=[pth,sc,'Tractography', sc, outname, '_cdti.trk'];
    outputMAT=[pth,sc,'Tractography', sc, outname, '_connectivity_tdti'];
    outputREP=[pth,sc,'Tractography', sc, outname, '_report_tdti'];
else
    outputTRK=[pth,sc,'Tractography', sc, out_name, '_tdti.trk'];
    outputTRK_C=[pth,sc,'Tractography', sc, out_name, '_cdti.trk'];
    outputMAT=[pth,sc,'Tractography', sc, out_name, '_connectivity_tdti'];
    outputREP=[pth,sc,'Tractography', sc, out_name, '_report_tdti'];
end
...
SelectVoxels=SelectVoxels(ROIsel(Smask(:)>sT)<1,:); 
...
disp(NF_all(end,end))
disp(' ')
disp('Time required for producing streamlines and matrices')
disp(timerun)
