function [AWF,Da,De,tortu,ADa,ADe,RDe,Kmax]=...
    run_DKI_single_fiber_model(D,K,V,Nvis,Uvis,optionsT)
...
DK=DirectionalKurt_V(D,K,V);

% step 2 - find maxima of KT
np=length(DK);
MeM=false(np,1);
for vt=1:np
    condi=(DK(vt)>DK(Uvis(vt,1:Nvis(vt))));
...
if isempty(MR) 
    ...
    [De,Da,DTe,DTa]=fun_compartmental_tensors(D,K,AWF);
    
    % step 8 - compute Da and De metrics 
    [ADa,ADe,RDe,tortu]=fun_tortuosity(DTe,DTa);
end
