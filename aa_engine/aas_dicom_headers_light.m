function hdr = aas_dicom_headers_light(P, essentials)
% Read limited header information from DICOM files
% This is entirely based on John Ashburner's spm_dicom_headers
% Rhodri Cusack 2011 UWO LondON
...
while ~isempty(tag) && ~(tag.group==65534 && tag.element==57357), % && tag.length==0),
    ...
    if tag.length>0,
        switch tag.name,
            case {'GroupLength','PixelData','CSAData'},
                % Ignore it
...
%                 ret.(tag.name) = dat;
            ...
            otherwise
                ...
                if ~isempty(tag.name),
                    ret.(tag.name) = dat;
                    if (isfield(ret,'SeriesNumber') && isfield(ret,'AcquisitionNumber'))
                        break
                    end;
