...
%         end
    case 'doit'
        ...
        switch aas_getsetting(aap,'overlay.template')
            case 'averaged_structurals'                
                ...
                if aas_stream_has_contents(aap,inpstreams{2})
                    
                    for s = 1:numel(aap.acq_details.subjects)
                        bgfname{s} = aas_getfiles_bystream(aap,s,inpstreams{2});
                    end
    ...
    case 'checkrequirements'
        ...
        inp = aas_getstreams(aap,'input'); inp(1:2) = []; % settings, structural
        for s = 1:numel(src)
            if strcmp(inp{(s-1)*2+1},src{s}), continue; end % every odd is result, every even is permuted
            if s == 1
