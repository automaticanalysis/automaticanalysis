function [aap resp]=aamod_emeg_groupsensoranalysis(varargin)
...
sensorvoldirs=regexprep(unique(regexprep(cellstr(enames),cellstr(eblocks),'BLOCK')),'.mat','');
...
for e=1:size(enames,1)
    ...
    %%% calculate grandmean
    S.Pout=fullfile(aap.acq_details.root,'GroupAnalysis_Sensors',[sprintf('g%g',size(S.P,1)) nam ext]);
    if exist(S.Pout,'file')==0 || settings.Overwrite==1
...
%% create group summaries of field topographies
    ...
    if ~isempty(regexp(D.fname,'-eeg','ONCE'))
        ...
        if ~exist(outfile,'file') || settings.Overwrite==1;
            aas_emeg_plot_topographies(char(S.P,S.Pout),'off',woi,'EEG','A4','subs x subs',outfile,0);
        end
    else
        ...
        if ~exist(outfile,'file') || settings.Overwrite==1;
            aas_emeg_plot_topographies(char(S.P,S.Pout),'off',woi,'Mags','A4','subs x subs',outfile,0);
        end
        ...
        if ~exist(outfile,'file') || settings.Overwrite==1;
            aas_emeg_plot_topographies(char(S.P,S.Pout),'off',woi,'Grads','A4','subs x subs',outfile,0);
        end
    end
...
%% plot grandmean ERFs?
    [path name]=fileparts(S.Pout);
    outfile=fullfile(path,'figures',[name '_sensors.ps']);
    if ~exist(outfile,'file') || settings.Overwrite==1;
        try 
            aas_emeg_plot_erf(S.Pout);
        catch
...
%% do repeated measures ANOVA with spm
 aas_emeg_rmANOVA_partitioned(aap,sensorvoldirs,true) %settings.Overwrite
 
 tfdirs={'mt1_sace_pifSTt4_BLOCK1_SampleK-mags';...
     'mt2_sace_pifSTt4_BLOCK1_SampleK-mags'};
%aas_emeg_rmANOVA_partitioned(aap,tfdirs,settings.Overwrite)

% final param for overwrite, although not fully implemented
 aas_emeg_plot_sensor_spms(aap,settings.Overwrite);
 
%fprintf('Done!'); keyboard;
