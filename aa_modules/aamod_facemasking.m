...
switch task
    ...
    case 'doit'
        ...
        toolsetup = sprintf('export FSLOUTPUTTYPE=NIFTI_PAIR;export MASKFACE_HOME=%s;export PATH=${MASKFACE_HOME}/bin:${PATH};',aap.directory_conventions.FaceMaskingdir);
        cmd = sprintf('fslmaths %s %s;cd %s;mask_face %s -a',...
            infname,...
            fullfile(wdir,spm_file(infname,'basename')),...
            fileparts(wdir),...
            fullfile(wdir,spm_file(infname,'basename')));
        aas_runfslcommand(aap,[toolsetup cmd]);
