% AA module
% [aap,resp]=aamod_tSNR_EPI(aap,task,p,s)
...
function [aap,resp]=aamod_tSNR_EPI(aap,task,p,s)
...
switch task
    ...
    case 'summary'
        sesspath=aas_getsesspath(aap,p,s);
    ...
    case 'doit'
        ...
        fprintf('\nWorking on session %s', aap.acq_details.sessions(s).name)
        ...
        % Save the SNR image!
        sV = V;
        sV.fname = fullfile(aas_getsubjpath(aap,p), ...
            ['tSNR_' aap.acq_details.sessions(s).name '.nii']);
        ...
        title(sprintf('\nSNR for session %s, using %.0f scans and TR %.3f', ...
            regexprep(aap.acq_details.sessions(s).name, '[^a-zA-Z0-9]', ''), ...
            size(EPIimg,1), ...
            K.RT))
        eval(legStr);
