...
switch task
    case 'doit'
        ...
        for p = 1:length(aap.acq_details.subjects)
            ...
            for s = 1:size(SEGimg,1)
                ...
                if p == 1
                    ...
                    booName = 0;
                    while booName == 0
                        if strcmp(segmentNames{s}, fn(1:length(segmentNames{s})))
                            booName = 1;
                        else
                            segmentNames{s} = segmentNames{s}(1:end-1);
                        end
                    end
                end
                ...
                V = spm_vol(SEGimg(s,:));
                structOverlay{s+1} = structOverlay{s+1} + spm_read_vols(V);
            end
        end
        ...
        for s = 1:size(SEGimg,1)
            V.fname = fullfile(aap.acq_details.root, [segmentNames{s} '_structOverlay.nii']);
            spm_write_vol(V, structOverlay{s+1}./length(aap.acq_details.subjects));
            
            outstream = strvcat(outstream, V.fname);
            
            %% Do some stats (maintly histograms of overlap...)
            histOverlay{1,s} = hist(structOverlay{1,s}(structOverlay{1,s}>0), ...
                1:length(aap.acq_details.subjects));
            % Normalise the histograms
            histOverlay{1,s} = histOverlay{1,s}./sum(histOverlay{1,s});    
            
            histOverlayA{1} = [histOverlayA{1} histOverlay{1,s}'];
                
            %% Also do an alternative measure...
            histOverlay{2,s} = histOverlay{1,s}.*(1:length(aap.acq_details.subjects));
            % Normalise the histograms
            histOverlay{2,s} = histOverlay{2,s}./sum(histOverlay{2,s});
            
            histOverlayA{2} = [histOverlayA{2} histOverlay{2,s}'];
        ...
        if aap.tasklist.currenttask.settings.diagnostic
            ...
            for y = 1:size(structOverlay{2},2)
                for s = 1:length(structOverlay)-1
                    h = subplot(1,length(structOverlay)-1,s);
                    imagesc(rot90(squeeze(structOverlay{s+1}(:,y,:))));
