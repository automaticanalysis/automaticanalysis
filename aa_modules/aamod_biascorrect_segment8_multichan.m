function [aap, resp] = aamod_biascorrect_segment8_multichan(aap, task, subjind)
...
% 
% Multichannel functionality (jt 03/July/2012):
%
% input stream:     (specified in xml file)
%
% output streams:   (input)     the bias-corrected t1 image
...
switch task
	case 'domain'
		resp = 'subject';
	case 'description'
		resp = 'SPM8 segment8 to bias correct image (multichannel)';
	case 'doit'
        ...
        % get the input image (general)
        %img = aas_getfiles_bystream(aap, subjind, 'structural');
        instream = aap.tasklist.currenttask.settings.inputstreams.stream;
        img = aas_getfiles_bystream(aap, subjind, instream);


        if isempty(img) || strcmp(img,'/')
            aas_log(aap, true, sprintf('Did not find a %s image.',instream));
        end
        ...
        outstream = aap.tasklist.currenttask.settings.outputstreams.stream{1};
        aap = aas_desc_outputs(aap, subjind, outstream, mimgfn);
