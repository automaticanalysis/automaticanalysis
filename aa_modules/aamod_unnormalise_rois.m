...
function [aap,resp] = aamod_unnormalise_rois( aap, task, p)
...
switch task
    case 'doit'
        ...
        try
            ...
            try
                SEGimg = aas_getfiles_bystream(aap,p,'segmasksStrict');
            catch
                try
                    SEGimg = aas_getfiles_bystream(aap,p,'segmasksZero');
                catch
                    SEGimg = [];
        ...
        invNormPar = aas_getfiles_bystream(aap,p,'normalisation_seg_inv_sn');
        % Normalisation flags...
        ...
        for r = 1:length(ROIlist)
            ...
            unix(['cp ' ROIlist{r} ' ' fullfile(aas_getsubjpath(aap,p), 'structurals') ]);
            
            [~, fn, ext] = fileparts(ROIlist{r});
            % New location for ROI...
            roi_fn = fullfile(aas_getsubjpath(aap,p), 'structurals', [fn, ext]);
            ...
            unix(['rm -rf ' roi_fn]);
            roi_fn = fullfile(aas_getsubjpath(aap,p), 'structurals', ['w' fn, ext]);
            
            % Reslice to EPI...
            spm_reslice(strvcat(mEPIimg, roi_fn), resFlags)
            % Delete ROI in MNI space...
            unix(['rm -rf ' roi_fn]);
            roi_fn = fullfile(aas_getsubjpath(aap,p), 'structurals', ['rw' fn, ext]);
            
            if ~isempty(SEGimg)
                % Now mask our ROIs by segmented mask (t for trimmed)
                conjMask(roi_fn, Mimg, [0 0.01], 't');
                unix(['rm -rf ' roi_fn]);
                roi_fn = fullfile(aas_getsubjpath(aap,p), 'structurals', ['trw' fn, ext]);
            end
