function [aap resp]=aamod_emeg_group_sources(varargin)
% Danny Mitchell 13/03/08 11/08
...
%%
gfile=fullfile(aap.acq_details.root,[mfilename '.parallel.mat']);
...
if isempty(varargin) || strcmp(varargin{2},'parallelise')
    ...
    for s=1:length(aap.acq_details.subjects)
        ...
        for b=aap.acq_details.selected_sessions
            ...
            temp=spm_select('List',subdir,settings.InputFilter);
    ...
    try blank=find(contrasts(:,1)==' ');
        ...
        aas_log(aap,true,sprintf('Found no data! Using filter: %s',settings.InputFilter));
    end
    contrasts(blank,:)=[];
    cblocks(blank,:)=[];
    ...
    loopvar=cellstr(cnames);
    save(gfile,'cfiles','loopvar');
...
% RELIC: zuloc/zul: zscored unsigned localisation (of contrast) (just +ve tail)
% RELIC: slocs: signed localisation of contrast (no longer used)
% scoul: signed contrast of unsigned localisation (both tails)
% scoul: signed contrast of signed localisation (both tails)
% RELIC: zucosl: zscored rectified contrast of signed localisation (just+ve
% tail)

% Run 2nd level stats for each tf-contrast image (e.g. one-sample t-tests)
...
for c=subblock;
    ...
    if strcmpi(settings.Stats,'auto')
        if size(SPM.xY.P,1)>13; settings.Stats='SPM';
    ...
    if exist('SPM.mat','file')==0 || settings.Overwrite==1
        ...
        if strcmpi(settings.Stats,'SPM')
            ...
            try
                ...
                debugnow % possibly no inmask voxels?
            end
    ...
    if ~exist(fullfile(aap.acq_details.root,'GroupAnalysis_Sources','figures',[cname '.png']),'file')||settings.Overwrite==1
        if ~isempty(strfind(cname,'_timecourses_')); return; end
        ...
        if any(ok2rend) % do the rendering at all thresholds
            ...
            ann1=annotation('textbox',[0 .94 1 .06],'Color','r','edge','none','String', ...
                {sprintf('%s: No significant voxels (even uncorrected) at p<%g per tail (%s).',lab,settings.thresh/2,settings.Stats), ...
...
return % ROI bit needs fixing for new file name format and parallelisation
